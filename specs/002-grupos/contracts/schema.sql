-- Contrato de schema: Grupos (002)
-- Fonte de verdade que a migration em supabase/migrations/ deve seguir.
-- Depende do schema da feature 001 (perfis, igrejas) já aplicado.
...
-- FR-003: criador vira automaticamente participante do próprio grupo (o
-- Dono é sempre também um participante — ver Edge Cases da spec).
...
-- WITH CHECK (true): sem isso, Postgres usaria USING também pra validar a
-- linha NOVA, e transferir a posse (dono_id != auth.uid() depois do UPDATE)
-- falharia. Quem pode transferir já é garantido pelo USING (só o dono atual
-- passa a cláusula); o trigger garante que o novo dono já participa.
