...
- [X] T019 **VENCIDA e refeita contra o critério certo, em 2026-08-10.** O texto abaixo é anterior à mudança que moveu a doxologia para o rodapé: SC-002 hoje diz "presente no rodapé da Home, **alcançável rolando até o fim**, em telas a partir de 375px de **largura**", e a própria spec registra "Deixa de ser exigida sem rolagem". Verificado no navegador: rolando até o fim, "A Deus seja a glória" aparece no rodapé, depois dos links legais. **Passou.** Não deu para testar 375px de largura porque o Chrome impõe largura mínima de janela (~600px); o leiaute é de coluna única, então largura menor só quebraria linha. Texto original: Conferir a interpretação de SC-002 registrada no risco 1 do plano: em paisagem a ~375px de altura, com **fonte padrão**, a doxologia está visível sem rolar; com fonte no máximo, nada é cortado e a página rola (FR-014). Se a doxologia não couber nem em fonte padrão, encurtar o bloco de identidade — não reduzir tamanho de fonte
- [X] T020 **REPROVOU e foi consertado em 2026-08-10.** Medidos no navegador, os botões da Home tinham 32 a 36px de altura — nenhum alcançava os 44 que SC-004 exige. Duas causas, e a segunda só apareceu na medição: `MaterialTapTargetSize` e, principalmente, o `visualDensity` adaptativo, que em desktop subtrai 8px e transformava um mínimo de 48 em 40 reais. Corrigido em `app_theme.dart`; remedido: mínimo de **48px** em todos os seis alvos. Contraste não remedido — `AppColors.navy` sobre branco segue em torno de 13:1. Original: Medir contraste dos pares texto/fundo e conferir alvos de toque (quickstart itens 7 e 8, FR-012, FR-013, SC-004). `AppColors.navy` sobre branco fica em torno de 13:1; a conferência importa se algum cinza claro tiver entrado
- [X] T021 **Verificado pela árvore de acessibilidade em 2026-08-10, não por leitor de tela real.** A ordem da árvore acompanha a visual — título, subtítulo, cartões, texto de chamada, botões, links legais — e termina em "A Deus seja a glória" como texto. VoiceOver pode se comportar diferente; se quiser certeza, é um item de dois minutos. Original: Conferir leitor de tela (quickstart item 10, FR-020, FR-024): ordem de leitura acompanha a ordem visual e "A Deus seja a glória" é lida como texto
