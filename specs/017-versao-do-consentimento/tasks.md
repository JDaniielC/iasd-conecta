...
- [X] T020 Rodar os quatro gates e **anotar os números reais**: `flutter analyze` (base 0 issues), `flutter test test/unit test/widget` (base **152**), `dart test test/integration` (base **127**), `flutter build web`. Confirmar que os **127** testes de integração pré-existentes passam **sem edição**, com a única exceção possível sendo a prevista em T001/T007
- [ ] T021 Rodar a Parte 2 de [quickstart.md](./quickstart.md), itens 1 a 12. **NÃO FEITA** — os dois itens que a própria tarefa aponta como sem substituto automatizado exigem rodar o app: o **item 6** (inspecionar o corpo do `insert` no DevTools para provar que o Dart não manda chave de versão) e o **item 1** (a tela de cadastro não ganhou campo nem passo). O que dá para afirmar sem isso: `toInsertMap` não tem chave de versão nenhuma (verificável por leitura, `profile.dart`), `profile_signup_page.dart` não foi tocado, e o teste de integração (b) prova que mesmo se o cliente mandasse uma versão ela seria descartada — o que torna o item 6 uma confirmação, não a única barreira
