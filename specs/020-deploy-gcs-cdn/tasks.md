...
- [ ] T005 Copiar `<PROJETO>`, `<BUCKET>` e `<URL_MAP>` de
...
- [ ] T006 Reescrever o cabeçalho de `.github/workflows/deploy-web.yml`:
      ...
      **`cancel-in-progress: false`**; `workflow_dispatch` ganha os inputs `target_bucket` e
      ...
      D-004). — **FR-005**
- [ ] T007 Escrever o step `Verify deploy secrets are present`, **antes** do build: falha
...
- [ ] T008 Preservar intacto o bloco que monta o `.env` de produção (`:34-42`) e o comentário
...
- [ ] T009 Acrescentar os steps `Authenticate to Google Cloud`
...
- [ ] T010 Escrever a **passada aditiva**: `gcloud storage rsync --recursive`
...
- [ ] T011 Escrever a **passada destrutiva**, logo depois: o mesmo `rsync` **com**
...
- [ ] T012 Garantir que build quebrado não publica: `flutter build web --release` vem
...
- [ ] T013 Escrever o step `Verify bundled .env carries only the two public keys`: extrai os
...
- [ ] T014 Excluir `.last_build_id` das duas passadas (`--exclude='^\.last_build_id$'`). É
      metadado de build, não faz parte do site.

**Checkpoint**: o fluxo publica no bucket de ensaio, completo e sem sobra. US1 testável sozinha.
...
- [ ] T015 Acrescentar, **depois** das duas passadas, o step `Set no-cache on index.html`:
...
- [ ] T016 Acrescentar o step `Invalidate CDN cache`:
...
- [ ] T017 Garantir que a falha de invalidação derruba o job. Três armadilhas, as três
...
- [ ] T018 Acrescentar o step `Report production state on failure` (`if: failure()`), que
...
**Checkpoint**: publicar sem invalidar é impossível de reportar como sucesso.
...
- [ ] T019 [P] [US3] Conferir que [quickstart.md](./quickstart.md) §1 e §3 listam **cada**
      ...
      `ADMIN_*`. — **FR-011**
- [ ] T020 [P] [US3] Conferir que [quickstart.md](./quickstart.md) §1 registra as permissões
...
- [ ] T021 [P] [US3] Conferir que [quickstart.md](./quickstart.md) §6 registra o tempo
...
- [ ] T022 [US3] Escrever a decisão sobre `dist-web` em
...
- [ ] T023 [US3] Acrescentar a seção **Deploy** ao `README.md`. Hoje o README **não descreve
...
- [ ] T024 Varrer os artefatos criados e o diff da feature atrás de **valor** de segredo:
...
- [ ] T025 Rodar `actionlint .github/workflows/deploy-web.yml`. Registrar a saída real
...
- [ ] T027 Remover o step `Publica build/web na branch dist-web`
      ...
      está no ar. — **FR-014**
- [ ] T028 Marcar os três itens de aceite de `.tickets/IASD-CI-GCS-UPLOAD.md:16-19` e
...
- [ ] T031 Registrar as lacunas conhecidas onde elas serão lidas depois (README ou
