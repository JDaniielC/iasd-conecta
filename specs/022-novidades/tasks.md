...
- [X] T025 **FEITO em 2026-08-10, no navegador.** Recarreguei o app (as 3 requisições de inicialização a `perfis?select=id` apareceram), limpei o registro de rede, e então abri `/novidades`: **zero requisições ao Supabase** ao renderizar os nove itens e gravar o marcador. O marcador ficou em `localStorage` como `flutter.novidades_ultima_vista`. Original: Executar a Parte 3 de [quickstart.md](./quickstart.md), itens 3.1 a 3.5. **O 3.1 é obrigatório**: com o DevTools na aba Network filtrando `supabase`, abrir a Home e a tela de Novidades e confirmar **zero** requisições. É a prova de FR-012 que nenhum teste automatizado dá
- [X] T026 **FEITO em 2026-08-10.** Sobrescrevi `localStorage.setItem`/`getItem` para lançar `SecurityError`, como em janela com armazenamento bloqueado, e naveguei até a tela: ela **abriu e listou os nove itens normalmente**, sem erro nem tela quebrada. Original: Executar o item 3.7 do quickstart — navegador com armazenamento bloqueado. A tela precisa **abrir e listar normalmente**; só o aviso é que reaparece a cada visita. Não pode dar erro. É o caso que `research.md` registra como não verificado

### O que só gente mede

- [ ] T027 **Metade automatizada FEITA**: a varredura de jargão roda sobre os nove textos reais e passa. **Falta a leitura pelas três pessoas do distrito — não tenho como substituí-la.** Antes: **Desbloqueada em 2026-08-10** — a varredura de jargão agora roda sobre oito textos reais e passa; falta a leitura pelas três pessoas do distrito. Antes: **Bloqueada até existir o primeiro item** — a varredura de jargão sobre a lista real já roda em `test/unit/novidades_test.dart` (grupo `CRITERIO-DE-NOVIDADE`) e passa vacuamente com a lista vazia; a leitura por três pessoas depende de haver o que ler. **SC-001/SC-002 com o conteúdo real**: quando o primeiro item entrar na lista, rodar a varredura de jargão sobre o **texto real** (`.dart`, nome de tabela, `v1.`, `RLS`, `policy`, `constraint`) e pedir a **três pessoas do distrito** que leiam e digam com as palavras delas o que mudou. Enquanto a lista estiver vazia, esta tarefa fica aberta — e é a primeira a fazer no dia do primeiro item
- [ ] T028 **NÃO FEITA — exige uma pessoa.** Cronometrar quem nunca viu a tela não é substituível por automação: eu sei onde o botão está. O que dá para afirmar é que ele está na Home, com rótulo em texto, no quarto lugar da lista de ações. **SC-006**: cronometrar alguém achando a tela a partir da Home, sem ajuda. Meta: menos de 15 segundos. Se passar, o problema é o caminho (FR-004), não a tela
