...
--
-- Conteúdo de specs/017-versao-do-consentimento/contracts/schema.sql, blocos 1
-- a 4, com UM desvio declarado (ver "DESVIO DO CONTRATO" abaixo).
--
-- DESVIO DO CONTRATO — o catálogo nasce com DUAS linhas, não uma
-- O contrato semeava só ('1.1', 2026-08-06), porque era a versão vigente quando
-- ele foi escrito. Entre o plano e esta migration, a feature 021 (visibilidade
-- do voto) mudou o texto da Política de Privacidade e subiu
-- LegalMetadata.version para '1.2', vigente desde 2026-08-09.
--
-- Semear só a 1.1 deixaria versao_texto_legal_vigente() devolvendo '1.1'
-- enquanto o Dart exibe '1.2' — divergência que
-- test/integration/versao_texto_legal_registro_test.dart existe justamente para
-- pegar, e que faria todo cadastro novo ser carimbado com a versão errada.
-- A 1.2 tem data de vigência documentada (legal_metadata.dart), então semeá-la
-- não é chute: é o mesmo critério que o contrato usou para incluir a 1.1 e
-- excluir a 1.0.
--
...
--
-- A 1.1 e a 1.2 entram porque as duas TÊM data documentada: 1.1 desde
-- 2026-08-06, e 1.2 desde 2026-08-09, quando a feature 021 mudou o texto da
-- Política. Critério idêntico, resultado diferente — é o que separa registro de
-- palpite.
insert into public.versoes_texto_legal (versao, vigente_desde) values
  ('1.1', timestamptz '2026-08-06 00:00:00-03'),
  ('1.2', timestamptz '2026-08-09 00:00:00-03');
...
begin
  ...
  --
  -- DESVIO DO CONTRATO, e ele importa. O contrato carimbava now() em TODO
  -- INSERT, sem olhar o que veio. Isso fabricava consentimento: quem
  -- inserisse um Perfil OMITINDO consentimento_lgpd_aceito_em recebia a data
  -- de graça, a coluna nunca ficava nula, e o `not null` que a feature 001 usa
  -- para tornar o consentimento obrigatório deixava de disparar. Uma feature de
  -- conformidade passaria a registrar aceite de quem nunca aceitou.
  --
  -- Pego por test/integration/perfis_constraints_test.dart, caso 'insert em
  -- perfis sem consentimento LGPD falha', que passou a não falhar mais.
  --
  -- A regra correta continua sendo "o cliente diz SE, o banco diz QUANDO": o
  -- SINAL é o campo vir preenchido; o VALOR é sempre substituído pelo now() do
  -- banco. Campo nulo é ausência de sinal, e segue nulo até bater no `not
  -- null`.
  if (tg_op = 'INSERT' and new.consentimento_lgpd_aceito_em is not null)
     or (tg_op = 'UPDATE'
         and new.consentimento_lgpd_aceito_em
             is distinct from old.consentimento_lgpd_aceito_em
         and new.consentimento_lgpd_aceito_em is not null) then
    new.consentimento_lgpd_aceito_em := now();
    new.consentimento_lgpd_versao := public.versao_texto_legal_vigente();
  elsif tg_op = 'INSERT' then
    -- Nada a carimbar. O `not null` da coluna recusa o insert.
    new.consentimento_lgpd_versao := null;
  else
