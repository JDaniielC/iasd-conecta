...
void main() {
  ...
  Future<int> coverCountFor(String column, Object id) async {
    final rows = await conn.execute(
      Sql.named(
        'select count(*) as total from public.fotos_capa where $column = @id',
      ),
      parameters: {'id': id},
    );
  ...
  tearDownAll(() async {
    ...
    //
    // A fila é limpa NO FIM, e não aqui, por dois motivos que já custaram
    // linhas residuais no banco local:
    //
    // 1. Os deletes de Ação e de Grupo abaixo cascateiam em `fotos_capa`, e o
    //    gatilho `fotos_capa_enfileirar_remocao` enfileira mais caminhos. Uma
    //    limpeza feita ANTES deles apaga o que existia e deixa o que ela mesma
    //    provocou.
    // 2. Os padrões antigos ('grupo/<meu grupo>/%' e 'acao/%-orfao-%') exigiam
    //    lembrar de cada caminho um a um, e o caso (g) usava
    //    'acao/<id>/intacta-capa.jpg', que não casava com nenhum dos dois.
    //
    // Hoje todo caminho deste arquivo termina em `-orfaotest.jpg`, e a fila sai
    // por esse sufixo, depois de tudo. O resíduo se acumulava justamente na
    // tabela cuja vacuidade é o sinal de saúde de SC-005.
    await conn.execute(
      Sql.named(
        'update public.rodadas_votacao set vencedora_id = null where grupo_id = @grupo',
      ),
    ...
    // Por último, depois de tudo que cascateia para `fotos_capa`.
    await conn.execute(
      Sql.named(
        'delete from public.capas_a_remover where caminho like @sufixo',
      ),
      parameters: {'sufixo': '%-orfaotest.jpg'},
    );
  ...
  test('(c) SC-005: candidata perdedora descartada leva a capa junto — o caminho '
      'que não passa por tela nenhuma', () async {
    ...
    await conn.execute(
      Sql.named(
        'update public.rodadas_votacao set vencedora_id = null where id = @rodada',
      ),
      parameters: {'rodada': roundId},
    );
    await conn.execute(
  ...
  test(
    '(d) FR-022: cancelar a Ação apaga a capa — não há cascade que dispare',
    () async {
      ...
      await asUser(_uidOwner, () async {
        ...
        await conn.execute(
          Sql.named(
            'update public.acoes set cancelada_em = now() where id = @id',
          ),
  ...
  test('(e) FR-023: Ação encerrada por tempo MANTÉM a capa — é histórico', () async {
  ...
  test('(f) FR-019: denúncias pendentes são encerradas quando a imagem some, '
      'por qualquer caminho', () async {
    ...
    expect(
      after.single.toColumnMap()['total'],
      0,
  ...
  test('(g) FR-013/FR-026/SC-009: remover a capa não altera NADA além dela', () async {
    ...
    Future<Map<String, int>> snapshot() async {
      final rows = await conn.execute(
        ...
        parameters: {'acao': candidateId, 'rodada': roundId, 'grupo': groupId},
      );
    ...
    expect(
      ...
      before,
      reason:
          'remover a capa não pode mexer em presença, voto, Ação, '
  ...
  test(
    ...
    () async {
      ...
      final path = 'grupo/$doomedGroupId/apagado-orfaotest.jpg';
      await conn.execute(
        Sql.named(
          'insert into public.fotos_capa (grupo_id, caminho, enviada_por) '
          'values (@g, @c, @d)',
      ...
      expect(
        ...
        isTrue,
        reason:
            'o cascade tem de disparar o gatilho, senão o arquivo fica '
  ...
  test(
    '(i) SC-005: a varredura alcança o arquivo que NUNCA teve linha, e só ele',
    () async {
      // O gatilho de enfileiramento dispara no DELETE de `fotos_capa`. Ele não
      // vê o arquivo que subiu e cuja linha nunca chegou a existir — a falha
      // do app entre o upload e o insert. Esse arquivo fica no bucket público
      // sem linha em lugar nenhum, e a consulta de saúde de INFRA-PRODUCAO.md
      // § 3 não se move, porque ela lê a fila.
      //
      // `varrer_capas_orfas()` é quem fecha isso. As três situações abaixo são
      // o que separa uma varredura útil de uma que apaga a capa de quem acabou
      // de enviá-la.
      //
      // **Este caso roda em transação revertida**, e não por preciosismo: o
      // Storage recusa `delete` direto nas suas tabelas —
      // *"Direct deletion from storage tables is not allowed. Use the Storage
      // API instead."* (medido em 2026-08-10, erro 42501). O `insert` passa,
      // o `delete` não, então uma linha inserida aqui ficaria no banco local
      // para sempre. O `rollback` no `finally` é a única limpeza possível — e
      // a recusa em si é a mesma verdade de research D-004 vista pelo outro
      // lado: SQL não mexe no bucket.
      final orfaoVelho = 'grupo/$groupId/varredura-velho-orfaotest.jpg';
      final orfaoNovo = 'grupo/$groupId/varredura-novo-orfaotest.jpg';
      final legitimo = 'grupo/$groupId/varredura-legitimo-orfaotest.jpg';

      await conn.execute('begin');
      try {
        // (1) Órfão com mais de uma hora: o caso real.
        await conn.execute(
          Sql.named(
            "insert into storage.objects (bucket_id, name, created_at) "
            "values ('fotos-capa', @c, now() - interval '3 hours')",
          ),
          parameters: {'c': orfaoVelho},
        );
        // (2) Órfão recém-chegado: é a janela NORMAL entre o upload e o insert.
        // Varrer isto seria apagar a capa de quem está enviando agora.
        await conn.execute(
          Sql.named(
            "insert into storage.objects (bucket_id, name, created_at) "
            "values ('fotos-capa', @c, now())",
          ),
          parameters: {'c': orfaoNovo},
        );
        // (3) Arquivo velho, mas referenciado: uma capa em uso.
        await conn.execute(
          Sql.named(
            "insert into storage.objects (bucket_id, name, created_at) "
            "values ('fotos-capa', @c, now() - interval '3 hours')",
          ),
          parameters: {'c': legitimo},
        );
        await conn.execute(
          Sql.named(
            'insert into public.fotos_capa (grupo_id, caminho, enviada_por) '
          ...
          parameters: {'g': groupId, 'c': legitimo, 'd': _uidOwner},
        );

        await conn.execute('select public.varrer_capas_orfas()');

        expect(
          await isQueued(orfaoVelho),
          isTrue,
          reason:
              'arquivo sem linha e sem fila é o órfão invisível — é '
              'exatamente o que a varredura existe para alcançar',
        );
        expect(
          await isQueued(orfaoNovo),
          isFalse,
          reason:
              'a carência de uma hora é o que impede a varredura de '
              'apagar um upload em andamento',
        );
        expect(
          await isQueued(legitimo),
          isFalse,
          reason: 'capa em uso não pode entrar na fila de remoção',
        );
      } finally {
  ...
  test(
    '(j) SC-010: pedir a drenagem também VARRE — o segundo gatilho do prazo',
    () async {
      // `pg_cron` roda dentro do Postgres, e projeto no plano gratuito é
      // pausado depois de uma semana sem uso: com o banco pausado, a varredura
      // agendada para de hora em hora para junto, e as 24 horas de SC-010
      // viram tempo indefinido. Por isso `drenar_capas_a_remover()` — que o
      // app chama a cada remoção e a cada troca — varre antes de qualquer
      // outra coisa.
      //
      // O "antes" importa: a função sai cedo quando a fila está vazia, e fila
      // vazia é EXATAMENTE o estado do órfão que nunca teve linha. Varrer
      // depois da saída antecipada seria não varrer nunca, justo no caso que
      // motivou a varredura.
      //
      // Transação revertida, como no caso (i): o Storage recusa `delete`
      // direto nas suas tabelas.
      final orfao = 'grupo/$groupId/segundo-gatilho-orfaotest.jpg';
      ...
      try {
        ...
        expect(await isQueued(orfao), isFalse);

        // **Como `authenticated`, e não como postgres.** O segundo gatilho
        // existe para o app, e o app nunca é superusuário: toda pessoa entra
        // com `signInAnonymously`, então o papel é sempre este.
        //
        // Não é formalidade. `varrer_capas_orfas()` tem `revoke all from
        // public` e nenhum grant para `authenticated` — funciona só porque
        // `drenar_capas_a_remover()` é `security definer` e a chamada interna
        // herda o dono. Provar isso como postgres seria provar o caminho que
        // ninguém percorre, o mesmo erro do primeiro achado de convergência
        ...
        expect(
          await isQueued(orfao),
          isTrue,
          reason:
              'quem acorda o banco é quem usa o app; se a drenagem '
              'pedida pelo app não varrer, o prazo de SC-010 depende de um '
              'cron que pode estar dormindo há uma semana',
        );
      } finally {
        await conn.execute('rollback');
      }
